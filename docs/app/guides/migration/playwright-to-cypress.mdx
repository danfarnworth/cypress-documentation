...
This guide helps you migrate from Playwright to Cypress.
...
Cypress does not control multiple browsers at the same time. For guidance and workarounds, see the [`@cypress/puppeteer`](https://www.npmjs.com/package/@cypress/puppeteer) plugin.
...
- **Auto Cancellation**: cancels a run early after failures to avoid spending CI time on a run that can’t pass. See [Auto Cancellation](/cloud/features/smart-orchestration/run-cancellation).
