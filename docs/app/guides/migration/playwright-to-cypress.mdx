...
- Configuration, filtering, and cross-browser run patterns
...
- Playwright: explicit waits are available when you need them → Cypress: most DOM queries/assertions wait and retry by default
- Playwright: `await page.waitForSelector(...)` → Cypress: `cy.get(...)` retries until it finds the element
- Playwright: `await expect(locator)...` → Cypress: `cy.get(...).should(...)` retries automatically
...
See Playwright [Frames](https://playwright.dev/docs/frames).

In Cypress, iframe support is more limited. If you need to test inside a third-party iframe, see the recipe [Testing iframes](https://github.com/cypress-io/cypress-example-recipes/tree/master/examples/blogs__iframes) and Cypress [Trade-offs](/app/references/trade-offs).
...
## Configuration mapping (high level)
...
See [Cypress Configuration](/app/references/configuration) and Playwright [Configuration](https://playwright.dev/docs/test-configuration).

## Filter and run a subset of tests
...
cypress run --spec "cypress/e2e/my-spec.cy.ts"
```

For title-based filtering in Cypress, see the recipe [Grep tests](https://github.com/cypress-io/cypress-example-recipes/tree/master/examples/preprocessors__grep) (Mocha-style grep).

## Cross-browser notes

Playwright commonly runs browsers via projects (`--project=chromium|firefox|webkit`). Cypress runs browsers via the `--browser` flag (and supports WebKit as experimental).
...
- Playwright equivalent: sharding is configured up front (`--shard=x/y`) and balancing depends on test structure and settings like `fullyParallel`. See Playwright [Sharding](https://playwright.dev/docs/test-sharding).
...
- Playwright equivalent: re-run last failures with `--last-failed`. See Playwright [Running tests](https://playwright.dev/docs/running-tests#run-last-failed-tests).
...
- Playwright equivalent: fail fast with `--max-failures=N` (stops after N failures). See Playwright [CLI](https://playwright.dev/docs/test-cli).
...
| `page.getByRole('button', { name: 'Save' })`                     | `cy.findByRole('button', { name: 'Save' })` (requires Cypress Testing Library) |
| `page.route('**/api', ...)`                                      | `cy.intercept('/api', ...)`                                                    |
| `await page.waitForResponse(...)`                                | `cy.intercept(...).as('x'); cy.wait('@x')`                                     |
| `await page.setInputFiles('input[type=file]', 'file.png')`       | `cy.get('input[type=file]').selectFile('cypress/fixtures/file.png')`           |
